...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--backend', choices=['sd', 'flux-kontext', 'flux-klein'], default='sd')
    parser.add_argument('--instance-manifest', help='Step2 JSON preserving each source image and parent category')
    parser.add_argument('--kontext-model', default='black-forest-labs/FLUX.1-Kontext-dev')
    parser.add_argument('--klein-model', default='/data/hujunjie/models/flux/klein-4b')
    parser.add_argument('--klein-steps', type=int, default=4)
    parser.add_argument('--klein-size', type=int, default=256)
    parser.add_argument('--same-parent-context-image', action='store_true', help='Klein: use the Step2 same-parent image and description as a second reference')
    parser.add_argument('--context-prompt-style', choices=['full', 'short'], default='full')
    parser.add_argument('--kontext-steps', type=int, default=28)
    parser.add_argument('--kontext-blocks-per-group', type=int, default=29)
    parser.add_argument('--kontext-limit', '--generation-limit', type=int, help='Run only the first N instances for a smoke test')
    parser.add_argument('--kontext-size', type=int, default=1024)
    parser.add_argument('--dry-run', action='store_true', help='Validate Kontext inputs without loading weights')
...
def main() -> None:
    ...
    args = parse_args()
    if args.backend in ('flux-kontext', 'flux-klein'):
        from pipeline.pipeline_flux import generate
        generate(args)
        return
