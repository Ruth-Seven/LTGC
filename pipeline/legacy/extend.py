...
def _run_signature(args, class_map, prompts, target):
    payload = {
        ...
        "prompt_pool_sha256": (
            _sha256_file(args.prompt_instance_description_path)
            if args.prompt_instance_description_path
            else None
        ),
        "step1_sources": {
            str(p): _sha256_file(p)
            for p in sorted(Path(args.step1_progress).glob("success_gpu*.csv"))
        }
        if args.step1_progress
        else None,
        "class_mapping": class_map,
        "prompts": prompts,
        "target": target,
        "batch_merge_num": args.batch_generate_num,
...
def load_step1_sources(progress_dir):
    """Index exact Step1 captions and retain every matching original image path.

    Reads the Step1 ``success_gpu*.csv`` files (label, image_path, description)
    and returns ``{(label, description): [image_path, ...]}`` so that a target or
    father description can be traced back to its real source image.
    """
    sources = {}
    parts = sorted(Path(progress_dir).glob("success_gpu*.csv"))
    if not parts:
        raise FileNotFoundError(f"No Step1 success CSVs in {progress_dir}")
    for part in parts:
        with part.open(newline="", encoding="utf-8") as source:
            for label, image_path, text in csv.reader(source):
                paths = sources.setdefault((str(label), text), [])
                if image_path not in paths:
                    paths.append(image_path)
    return sources


def source_instance(label, text, class_map, sources=None):
    """Build a description instance record, optionally resolved to a real image."""
    _, parent, display = parse_semantic_label(_get_class_name(label, class_map))
    record = dict(label=str(label), class_name=display, parent_category=parent, descriptions=[text])
    if sources is not None:
        paths = sources[(str(label), text)]
        image_path = choice(paths)
        if not Path(image_path).is_file():
            raise FileNotFoundError(image_path)
        record.update(image_path=image_path, matching_image_paths=list(paths))
    return record


def _sample_prompt_instances(all_existing, target_label, number, class_map, sources=None):
    """Sample same-parent (father) classes with replacement and one real description per draw.

    Each draw resolves to a single real father-class description plus, when Step1
    sources are available, its original image path.
    """
    ...
    return [source_instance(label, choice(texts), class_map, sources) for label, texts in draws]
...
def _extend_worker(
    ...
    for idx, (label, texts) in enumerate(class_list):
        ...
        # anchor (原描述) and one random father-class description (父类随机描述).
        # Generated history stays out of model context to avoid cross-round copying.
        for ti in range(max_attempts):
            if len(all_new) >= description_target:
                break
            reference_text = texts[randint(0, n_existing - 1)]
            target_instance = source_instance(label, reference_text, class_map, step1_sources)
            prompt_instances = _sample_prompt_instances(
                all_existing, label, per_text, class_map, step1_sources
            )
            logger.info(
            ...
            for description in accepted:
                ...
                provenance.append(
                    dict(
                        ...
                        source_prompt=reference_text,
                        extended_prompt=description,
                        target_instance=target_instance,
                        father_instances=prompt_instances,
                    )
                )
        ...
        if len(all_new) == description_target:
            ...
            atomic_json_dump(provenance, os.path.join(progress_dir, "provenance", f"{label}.json"))
...
def parse_args():
    ...
    parser.add_argument(
        "--step1-progress",
        default=None,
        help="Step1 progress directory with success_gpu*.csv; resolve target/father original image paths",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--run-dir", help="Flux manifest mode: extend sample_manifest.json and extract father scene"
    )
...
def main():
    ...
    args = parse_args()
    if args.run_dir:
        from pipeline.pipeline_flux import extend

        extend(args)
        return
    if args.target_number is None and args.fixed_number is None:
        raise ValueError(
            "Provide --target-number or --fix-number for CSV mode, or --run-dir for Flux mode"
        )
    ...
    step1_sources = load_step1_sources(args.step1_progress) if args.step1_progress else None
    if step1_sources is not None:
        for label, texts in grouped + list(all_existing.items()):
            for text in texts:
                if (str(label), text) not in step1_sources:
                    raise KeyError(f"No Step1 original image for {label}: {text}")
    ...
    # ── 汇聚 provenance：每条扩展描述 → 原描述 + 父类描述 + 父类描述图片 path ──
    provenance = []
    for label, _ in target_grouped:
        with open(
            os.path.join(progress_dir, "provenance", f"{label}.json"), encoding="utf-8"
        ) as source:
            records = json.load(source)
        if [r["extended_prompt"] for r in records] != completed[str(label)]:
            raise RuntimeError(f"Step2 provenance does not match class {label}")
        provenance.extend(records)
    atomic_json_dump(provenance, args.extended_description_path + ".provenance.json")

    # ── 扁平 CSV：label, class_name, 扩展描述, 原描述, 原描述图片, 父类 label/class/描述/图片 path ──
    flat_path = args.extended_description_path + ".with_parent.csv"
    flat_tmp = flat_path + ".tmp"
    with open(flat_tmp, "w", newline="", encoding="utf-8") as output:
        writer = csv.writer(output)
        writer.writerow(
            [
                "label",
        ...
        for record in provenance:
            target = record["target_instance"]
            for father in record["father_instances"]:
                writer.writerow(
                    [
                        record["label"],
                        record["class_name"],
                        record["extended_prompt"],
                        record["source_prompt"],
                        target.get("image_path", ""),
                        father["label"],
                        father["class_name"],
                        father["descriptions"][0],
                        father.get("image_path", ""),
                    ]
                )
    ...
    logger.info(
        "Done. Output: %s (+ .provenance.json, .with_parent.csv)", args.extended_description_path
    )
